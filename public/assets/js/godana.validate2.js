			            ...
			            if ($this.val() != '') {
			            	$.ajax({
			    				...
			    				beforeSend: function() {    
			            			...
			            			$this.parents('label').removeClass('register-valid').removeClass('register-error');
			            			$this.parents('label').find('.help-block').remove();			
			            		},
			                    success: function(res) {
			                        if (!res.success) {
			                        	$(settings.loadingClass).html("<i class='icon-remove text-error'></i>");                        	
			                        	$this.parents('label').removeClass('register-valid').addClass('register-error');
			                        	$this.parents('label').append(res.error_msg);
//			                        	$(settings.btnRegister).addClass('disabled');
			                        } else {
			                        	$(settings.loadingClass).html("<i class='icon-ok text-success'></i>");
			                        	$this.parents('label').removeClass('register-error').addClass('register-valid');
			                        	$this.parents('label').find('.help-block').remove();
